...
public:
    ...
    void create(const std::vector<float> &vertices, int32_t count, const std::vector<uint32_t> &indices);
    void create(const std::vector<VK_Vertex> &vertices, const std::vector<uint32_t> &indices);
...
private:
                                 ...
                                 bool vertex)
    {
        ...
        context->createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | (vertex ? VK_BUFFER_USAGE_VERTEX_BUFFER_BIT :
